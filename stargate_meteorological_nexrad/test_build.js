/**
 * Automated Verification Test Suite for stargate_meteorological_nexrad
 * Port: 8086
 */
...
async function runTests() {
  console.log('=== STARTING AUTOMATED VERIFICATION: stargate_meteorological_nexrad ===');
  ...
  try {
    ...
    const glyphs = [
      '.glyph-btn[data-glyph="SUPERCELL-MESO"]',
      '.glyph-btn[data-glyph="CYCLONE-VORTEX"]',
      '.glyph-btn[data-glyph="BAROMETRIC-SHEAR"]',
      '.glyph-btn[data-glyph="ISOBAR-GRADIENT"]',
      '.glyph-btn[data-glyph="MICROBURST-DOWNDRAFT"]',
      '.glyph-btn[data-glyph="CORIOLIS-FRONT"]',
      '.glyph-btn[data-glyph="SQUALL-LINE"]'
    ];
    ...
    if (state7Locks.teleState.includes('STORM EYE CONDUIT ACTIVE')) {
    ...
    await hitAndClick('.preset-btn[data-preset="moore-ef5"]');
    await sleep(2600); // storm-track replay: 7 vectors x 280ms sweep cadence
    ...
    if (stateAfterPreset.teleState.includes('STORM EYE CONDUIT ACTIVE')) {
    ...
    console.log('Clicking primary ENGAGE CYCLONE VORTEX CONDUIT button...');
    await hitAndClick('#btn-activate');
    await sleep(1000);

    const stateActive = await cdp.eval(`(() => {
      const teleState = document.getElementById('tele-state').textContent;
      const eye = document.getElementById('eye-status').textContent;
      return { teleState, eye };
    })()`);
    console.log(`[Cycle 2 - Active State]:`, stateActive);
    if (!stateActive.teleState.includes('STORM EYE CONDUIT ACTIVE')) {
      throw new Error('FAIL: Manual activation did not transition to STORM EYE CONDUIT ACTIVE.');
    }
    ...
    // --- TEST STAGED STORM-TRACK REPLAY (per-vector locks, no instant jump) ---
    ...
    const stagedOk = rCounts.every((v, i) => i === 0 || v > rCounts[i - 1]) &&
      replaySamples.every(s => s.locked > 0 && s.locked < 7 && s.activateDisabled &&
        !s.teleState.includes('STORM EYE CONDUIT ACTIVE'));
    if (!stagedOk) {
    ...
    if (!replayEnd.counter.includes('7 / 7') || replayEnd.activateDisabled ||
        replayEnd.teleState.includes('STORM EYE CONDUIT ACTIVE')) {
      throw new Error('FAIL: Replay did not land in PENDING_READY without auto-fire.');
    }
    console.log('>>> STAGED REPLAY CONFIRMED: per-vector locks, PENDING at end, no auto-fire.');
    ...
    await sleep(200);
    await hitAndClick('.preset-btn[data-preset="moore-ef5"]');
    await sleep(500); // ~2 vectors locked (250ms already slept in hitAndClick)
    ...
    await hitAndClick('#btn-disengage');
    await sleep(1900); // past when remaining sweeps would have fired
    ...
    if (!afterAbortState.includes('0 / 7')) {
      throw new Error('FAIL: Abort during replay left orphaned sweep timers still locking vectors.');
    }
    ...
    // --- TEST BAROMETRIC OVERPRESSURE SAFETY HOLD ---
    console.log('\n--- TESTING BAROMETRIC OVERPRESSURE SAFETY HOLD ---');
    ...
    await hitAndClick('.glyph-btn[data-glyph="SUPERCELL-MESO"]');
    await sleep(300);
    ...
    if (!bannerActive) {
      throw new Error('FAIL: Overpressure Safety Hold did not trigger interlock banner on blocked dial.');
    }
    ...
    console.log('\n=== ALL VERIFICATION TESTS PASSED FOR stargate_meteorological_nexrad ===\n');
