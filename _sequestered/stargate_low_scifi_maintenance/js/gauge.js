...
export function makeGauge({ label, unit, min, max, zones = [], ticks = 10 }) {
  ...
  inner += `
    <g id="needle" transform="rotate(0 ${cx} ${cy})">
      <line x1="${cx}" y1="${cy + 4}" x2="${cx}" y2="${cy - rFace + 10}" stroke="#14100c" stroke-width="2.2" stroke-linecap="round"/>
      <circle cx="${cx}" cy="${cy}" r="5.5" fill="#14100c"/>
      <circle cx="${cx}" cy="${cy}" r="2" fill="#3a332a"/>
    </g>
    <rect x="4" y="81" width="112" height="9" rx="2.5" fill="#0f0d0a" opacity=".4"/>
    <text x="${cx}" y="88" font-size="6.2" text-anchor="middle" fill="#c9c2ab" letter-spacing="1.2" font-weight="bold">${label} &#183; ${unit}</text>`;
