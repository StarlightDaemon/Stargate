/**
 * Automated Verification Test Suite for stargate_epigraphic_spectrometry
 * Port: 8082
 */
...
async function runTests() {
  console.log('=== STARTING AUTOMATED VERIFICATION: stargate_epigraphic_spectrometry ===');
  ...
  try {
    ...
    // --- TEST CYCLE 1: DIAL 7 GLYPHS -> VERIFY NEGATIVE AUTO-FIRE -> DISENGAGE ---
    console.log('\n--- EXECUTING DIAL-DISENGAGE CYCLE 1 ---');
    const glyphs = [
      '.glyph-btn[data-glyph="KHEPRI-SOL"]',
      '.glyph-btn[data-glyph="ANUBIS-NEB"]',
      '.glyph-btn[data-glyph="HORUS-AETHEL"]',
      '.glyph-btn[data-glyph="ISIS-NEXUS"]',
      '.glyph-btn[data-glyph="THOTH-CHRONO"]',
      '.glyph-btn[data-glyph="OSIRIS-HADAL"]',
      '.glyph-btn[data-glyph="PTAH-STRUCT"]'
    ];
    ...
    const state7Locks = await cdp.eval(`(() => {
    ...
    console.log(`[Cycle 1 - 7 Locked State]:`, state7Locks);

    // NEGATIVE ASSERTION CHECK
    if (state7Locks.teleState.includes('GOLDEN CONDUIT OPEN')) {
      throw new Error('FATAL FAILURE: Gate auto-fired on 7th locked symbol!');
    }
    if (state7Locks.activateDisabled) {
      throw new Error('FAIL: Activate button was not enabled when 7 symbols locked.');
    }
    console.log('>>> CONFIRMED NEGATIVE CASE: 7th symbol locked, system is in PENDING state and did NOT auto-activate.');

    // Disengage Cycle 1
    console.log('Disengaging via Purge button...');
    ...
    if (!stateAfterDisengage1.includes('0 / 7')) {
    ...
    await hitAndClick('.preset-btn[data-preset="rosetta-quantum"]');
    await sleep(2600); // cartouche replay: 7 glyphs x 280ms scan cadence
    ...
    if (stateAfterPreset.teleState.includes('GOLDEN CONDUIT OPEN')) {
      throw new Error('FATAL FAILURE: Preset auto-activated the gate!');
    }
    console.log('>>> CONFIRMED NEGATIVE CASE: Preset loaded 7 symbols into PENDING state without auto-activating.');

    // Now Activate explicitly
    console.log('Clicking primary INITIATE EPIGRAPHIC HARMONIC button...');
    await hitAndClick('#btn-activate');
    await sleep(1000);

    const stateActive = await cdp.eval(`(() => {
      const teleState = document.getElementById('tele-state').textContent;
      const cartouche = document.getElementById('cartouche-status').textContent;
      return { teleState, cartouche };
    })()`);
    console.log(`[Cycle 2 - Active State]:`, stateActive);
    if (!stateActive.teleState.includes('GOLDEN CONDUIT OPEN')) {
      throw new Error('FAIL: Manual activation did not transition to GOLDEN CONDUIT OPEN.');
    }

    await cdp.screenshot(path.join(__dirname, 'screenshot_active.png'));

    // Disengage active conduit
    console.log('Disengaging active conduit via Purge button...');
    await hitAndClick('#btn-disengage');
    await sleep(400);
    ...
    // --- TEST STAGED CARTOUCHE REPLAY (per-glyph locks, no instant jump) ---
    ...
    const stagedOk = rCounts.every((v, i) => i === 0 || v > rCounts[i - 1]) &&
      replaySamples.every(s => s.locked > 0 && s.locked < 7 && s.activateDisabled &&
        !s.teleState.includes('GOLDEN CONDUIT OPEN'));
    if (!stagedOk) {
    ...
    const toEnd = clickT0 + 1960 + 1200 - Date.now();
    ...
    if (!replayEnd.counter.includes('7 / 7') || replayEnd.activateDisabled ||
        replayEnd.teleState.includes('GOLDEN CONDUIT OPEN')) {
      throw new Error('FAIL: Replay did not land in PENDING_READY without auto-fire.');
    }
    console.log('>>> STAGED REPLAY CONFIRMED: per-glyph locks, PENDING at end, no auto-fire.');

    // --- TEST DISENGAGE DURING REPLAY ---
    console.log('\n--- TESTING PURGE DURING PRESET REPLAY ---');
    await hitAndClick('#btn-disengage');
    await sleep(200);
    await hitAndClick('.preset-btn[data-preset="rosetta-quantum"]');
    await sleep(500); // ~2 glyphs locked (250ms already slept in hitAndClick)
    const midAbortState = await cdp.eval(`document.getElementById('address-counter').textContent`);
    console.log(`[Mid-replay before purge]: ${midAbortState}`);
    await hitAndClick('#btn-disengage');
    await sleep(1900); // past when remaining scans would have fired
    const afterAbortState = await cdp.eval(`document.getElementById('address-counter').textContent`);
    console.log(`[After mid-replay purge +1.9s]: ${afterAbortState}`);
    if (!afterAbortState.includes('0 / 7')) {
      throw new Error('FAIL: Purge during replay left orphaned scan timers still locking glyphs.');
    }
    await cdp.screenshot(path.join(__dirname, 'screenshot_mid_replay_abort.png'));
    console.log('>>> MID-REPLAY PURGE CONFIRMED: no orphaned locks after disengage.');

    // --- TEST LASER OVERLOAD SAFETY HOLD ---
    console.log('\n--- TESTING LASER OVERLOAD SAFETY HOLD ---');
    ...
    await hitAndClick('.glyph-btn[data-glyph="KHEPRI-SOL"]');
    await sleep(300);
    ...
    if (!bannerActive) {
      throw new Error('FAIL: Laser Overload Hold did not trigger interlock banner on blocked dial.');
    }
    ...
    console.log('\n=== ALL VERIFICATION TESTS PASSED FOR stargate_epigraphic_spectrometry ===\n');
