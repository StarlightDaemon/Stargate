...
> **Status: closed, complete, no further iteration planned.** The build is functionally solid and this is not a defect close-out. Automated Puppeteer verification passed 109/109 (hit-tested real events, screenshot-measured stages), and direct hands-on testing in an interactive browser confirmed the core interactions work: ward selection turns and catches, a remembered-hand quick-dial runs to unbarred one ward at a time, and the three-stage bell sequence opens the gate. One documented caveat: a CSS transition (the gate bar fading out after unbarring) appeared mid-fade in a screenshot taken from a throttled interactive browser pane, while headless Chrome and the computed styles showed it completing normally; this was resolved as an environment timing artifact, not a build bug. **Why it is closed:** a creative and genre-fit judgment by the operator — rendering an inherently supernatural subject through this gallery's standard glowing-schematic register undercuts the theme itself for the gallery's purposes. The build works as designed; it is closed because the concept and the house register pull against each other here, not because anything is broken.
