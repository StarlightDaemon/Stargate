/**
 * Automated Verification Test Suite for stargate_genomic_sequencer
 * Port: 8085
 */
...
async function runTests() {
  console.log('=== STARTING AUTOMATED VERIFICATION: stargate_genomic_sequencer ===');
  ...
  try {
    ...
    const glyphs = [
      '.glyph-btn[data-glyph="ADENINE-α"]',
      '.glyph-btn[data-glyph="CYTOSINE-β"]',
      '.glyph-btn[data-glyph="GUANINE-γ"]',
      '.glyph-btn[data-glyph="THYMINE-δ"]',
      '.glyph-btn[data-glyph="URACIL-ε"]',
      '.glyph-btn[data-glyph="INOSINE-ζ"]',
      '.glyph-btn[data-glyph="PSEUDOURIDINE-ψ"]',
      '.glyph-btn[data-glyph="METHYLGUANINE-μ"]'
    ];
    ...
    if (state8Locks.teleState.includes('QUANTUM HELIX ACTIVE')) {
    ...
    // Disengage Cycle 1
    console.log('Disengaging via Flush button...');
    ...
    await hitAndClick('.preset-btn[data-preset="syn-ch-1"]');
    await sleep(2600); // thermocycler replay: 8 loci x 260ms cycle cadence
    ...
    if (stateAfterPreset.teleState.includes('QUANTUM HELIX ACTIVE')) {
    ...
    console.log('Clicking primary SYNTHESIZE QUANTUM HELIX button...');
    await hitAndClick('#btn-activate');
    await sleep(1000);

    const stateActive = await cdp.eval(`(() => {
      const teleState = document.getElementById('tele-state').textContent;
      const helicase = document.getElementById('helicase-status').textContent;
      return { teleState, helicase };
    })()`);
    console.log(`[Cycle 2 - Active State]:`, stateActive);
    if (!stateActive.teleState.includes('QUANTUM HELIX ACTIVE')) {
      throw new Error('FAIL: Manual activation did not transition to QUANTUM HELIX ACTIVE.');
    }

    await cdp.screenshot(path.join(__dirname, 'screenshot_active.png'));

    // Disengage active conduit
    console.log('Disengaging active conduit via Flush button...');
    await hitAndClick('#btn-disengage');
    await sleep(400);
    ...
    // --- TEST STAGED THERMOCYCLER REPLAY (per-locus locks, no instant jump) ---
    ...
    const stagedOk = rCounts.every((v, i) => i === 0 || v > rCounts[i - 1]) &&
      replaySamples.every(s => s.locked > 0 && s.locked < 8 && s.activateDisabled &&
        !s.teleState.includes('QUANTUM HELIX ACTIVE'));
    if (!stagedOk) {
    ...
    if (!replayEnd.counter.includes('8 / 8') || replayEnd.activateDisabled ||
        replayEnd.teleState.includes('QUANTUM HELIX ACTIVE')) {
      throw new Error('FAIL: Replay did not land in PENDING_READY without auto-fire.');
    }
    console.log('>>> STAGED REPLAY CONFIRMED: per-locus locks, PENDING at end, no auto-fire.');
    ...
    await sleep(200);
    await hitAndClick('.preset-btn[data-preset="syn-ch-1"]');
    await sleep(450); // ~2 loci locked (250ms already slept in hitAndClick)
    ...
    await hitAndClick('#btn-disengage');
    await sleep(2000); // past when remaining cycles would have fired
    const afterAbortState = await cdp.eval(`document.getElementById('address-counter').textContent`);
    console.log(`[After mid-replay abort +2s]: ${afterAbortState}`);
    if (!afterAbortState.includes('0 / 8')) {
      throw new Error('FAIL: Abort during replay left orphaned cycle timers still locking loci.');
    }
    ...
    // --- TEST PCR THERMAL OVERHEAT HOLD ---
    console.log('\n--- TESTING PCR THERMAL OVERHEAT HOLD ---');
    ...
    await hitAndClick('.glyph-btn[data-glyph="ADENINE-α"]');
    await sleep(300);
    ...
    if (!bannerActive) {
      throw new Error('FAIL: Thermal Overheat Hold did not trigger interlock banner on blocked dial.');
    }
    ...
    console.log('\n=== ALL VERIFICATION TESTS PASSED FOR stargate_genomic_sequencer ===\n');
